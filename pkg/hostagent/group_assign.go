...
func (agent *HostAgent) assignErspanPolicies(pod *v1.Pod) ([]metadata.OpflexGroup, error) {
	...
	// Add erspan policies that directly select this pod
	for _, spankey := range agent.erspanPolPods.GetObjForPod(podkey) {
		g = addGroup(gset, g, agent.config.DefaultEg.PolicySpace,
			util.AciNameForKey(agent.config.AciPrefix, "span", spankey))
	}
	return g, nil
}
